...
用户要求选股、新增候选、收盘总结时，读取 [市场发现与报告规范](references/market-review.md)。先看市场和新机会，再复核原 watchlist；不得把仅覆盖原名单的 cycle 输出称为全市场选股。

- 用当日完整涨停池和前一交易日对比板块扩散、连板梯队；区分行业归类与题材主线。
- 以当日连板数、价格和公告纠正计划中的过时描述；高位连板不能沿用旧的“首板/二板”策略标签。只指出计划差异，未经授权不改交易参数。
- 新候选分为研究观察、板块强弱参照、满足条件后的买入候选；退潮时也要寻找和解释观察机会，但不得绕过买入闸门。
- 将扩展候选的最新行情与完整市场背景交给 Jev 复核，标明分析范围和与前轮结论的变化；Jev 不能替代硬规则。
- 每份总结包含市场变化、新增/保留/降权名单、入场与失效条件、数据缺口。报告按用户已授权渠道发送，并分别核对回执。
...
## 纪律（每次回复末尾附带）
首测试错0.5成、单票≤2成，总仓服从计划与情绪上限；止损触发时按可卖数量执行，当日买入受 T+1 限制的仓位须标记为下一交易日优先处理。技术研究辅助，不构成投资建议。
