...
每个有效交易时段轮次，以及用户要求选股、新增候选、收盘总结时，必须读取 [市场发现与报告规范](references/market-review.md)。先看市场和新机会，再复核原 watchlist；不得把仅覆盖原名单的 cycle 输出称为全市场选股。

- 每轮更新独立研究观察池，读取前轮归档，覆盖完整涨停/炸板池及前轮未涨停候选，核验最新行情和公告并交给 Jev；明确新增、保留、降权、移出和失效条件。退潮暂停买入不暂停发现，无变化仍归档但不重复推送。普通 cycle 不会自动完成本步骤。
- 用当日完整涨停池和前一交易日、当日基线对比板块扩散、连板梯队；区分行业归类与题材主线。
...
## 规则细节
- MA5±1%是本系统的低吸辅助区间，不是陈小群本人固定阈值；先判断主线和核心地位，再看技术位置。
- 涨停股允许有条件的排板/回封买入候选提醒，具体依据见市场发现规范；需通过行情证据、Jev主线/核心复核和所有硬闸门。五分钟快照不证明成交机会或即时撤单能力，不自动委托。
